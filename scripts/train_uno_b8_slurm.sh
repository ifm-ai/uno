...
if (( $# > 1 )); then
  echo "Usage: bash scripts/train_uno_b8_slurm.sh [epochs]" >&2
  exit 2
fi
...
TRAIN_EPOCHS="$(${PYTHON_BIN} -c \
  'import sys; from nano_vllm_uno.training.create_fixed_curriculum import parse_epochs; print(format(parse_epochs(sys.argv[1]), "f"))' \
  "${TRAIN_EPOCHS}")"
...
"${PYTHON_BIN}" -m nano_vllm_uno.training.create_fixed_curriculum \
...
max_steps="$(${PYTHON_BIN} -c \
  'import sys; from nano_vllm_uno.training.curriculum import BlockCurriculumPlan; print(BlockCurriculumPlan.from_yaml(sys.argv[1]).max_steps)' \
  "${CURRICULUM_PATH}")"
