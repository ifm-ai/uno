# Portions of this file are adapted from LiveCodeBench:
# https://github.com/LiveCodeBench/LiveCodeBench/tree/main/lcb_runner/evaluation
...
def reliability_guard(maximum_memory_bytes: Optional[int] = None):
    ...
    generated code, should not be blindly executed outside of one. Use an
    isolated sandbox and proceed with caution.
    """
...
def check_correctness(sample, generation, timeout: int) -> list[int | bool]:
    ...
    def _temp_run(sample, generation, result_queue):
        result_queue.put(run_test(sample, test=generation, timeout=timeout))

    result_queue = multiprocessing.Queue(maxsize=1)
    p = multiprocessing.Process(target=_temp_run, args=(sample, generation, result_queue))
    p.start()
