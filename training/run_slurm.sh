...
DEEPSPEED_PATH="${DEEPSPEED_PATH:-${REPO_ROOT}/training/configs/deepspeed_zero2.json}"
...
if [[ "${UNO_NODE_WORKER:-0}" == "1" ]]; then
  ...
  export XDG_CACHE_HOME="${XDG_CACHE_HOME:-${worker_tmpdir}/cache}"
  export TRITON_CACHE_DIR="${TRITON_CACHE_DIR:-${worker_tmpdir}/triton}"
...
echo "Prefetching the pinned Uno Qwen base into ${HF_CACHE_DIR}"
srun --nodes=1 --ntasks=1 "${PYTHON_BIN}" -m training.prefetch_model \
