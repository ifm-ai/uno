...
def _grade_math_row(row: dict[str, Any], grader: str) -> dict[str, Any]:
...
def grade_gsm8k_row(row: dict[str, Any]) -> dict[str, Any]:
    return _grade_math_row(row, "gsm8k")


def grade_math500_row(row: dict[str, Any]) -> dict[str, Any]:
    return _grade_math_row(row, "math500")


def grade_aime_row(row: dict[str, Any]) -> dict[str, Any]:
    return _grade_math_row(row, "aime")


def grade_math_row(row: dict[str, Any]) -> dict[str, Any]:
    """Backward-compatible generic math grader."""

    return _grade_math_row(row, "math")


def score_math(
    rows: list[dict[str, Any]], task: str | None = None
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    graders = {
        "gsm8k": grade_gsm8k_row,
        "math500": grade_math500_row,
        "aime24": grade_aime_row,
        "aime25": grade_aime_row,
        "aime26": grade_aime_row,
    }
    grade_row = graders.get(task, grade_math_row)
    ...
    summary = {
        "grader": graded[0]["grader"] if graded else task or "math",
        "num_rows": len(graded),
